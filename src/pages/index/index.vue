...
} from '../../components/generator/html' // 引入生成HTML、模板、脚本和样式的方法
...
// 供拉拽生成的组件
import { inputComponents, selectComponents, formConf } from "../../components/generator/formConfig" // 引入输入型、选择型组件和表单配置

// 默认值
// import darwDefault from "../../components/generator/darwDefault" // 默认值导入（已注释）

const leftComponents = [ // 左侧组件列表
  {
    title: '输入型组件',
...
const onEnd = (obj: any) => { // 拖拽结束时的回调
  if (obj.to !== obj.form) {
    console.log(true)
  }
}

const cloneComponent = (data: any) => { // 克隆组件
...
const activeFormItem = (currentItem: any) => { // 激活表单项
...
const drawingItemCopy = (item: any, list: any) => { // 复制绘图项
...
watch(() => activeId.value, val => { // 监听activeId的变化
  console.log(val)
 ...
 * 创建formId 并强制更新组件
 */
const createIdAndKey = (item: any) => { // 创建ID和Key
 ...
 * 删除预览的某一个拖拽组件
 */
const drawingItemDelete = (index: any, list: any[]) => { // 删除绘图项
  list.splice(index, 1)
  nextTick(() => {
