...
const emit = defineEmits(['tag-change', 'fetch-data'])

const currentTab = ref<string>('field')

// 文档地址
const documentLink = computed(() => props.activeData.__config__.document || 'https://element.eleme.cn/#/zh-CN/component/installation')
...
const activeTag: any = computed(() => props.activeData.__config__.tag)

const isShowMin: any = computed(() => ['el-input-number', 'el-slider'].indexOf(activeTag) > -1)
const isShowMax: any = computed(() => ['el-input-number', 'el-slider', 'el-rate'].indexOf(activeTag) > -1)
const isShowStep: any = computed(() => ['el-input-number', 'el-slider'].indexOf(activeTag) > -1)
...
const tagChange = (tagIcon: string) => {
  let target: any = inputComponents.find(item => item.__config__.tagIcon === tagIcon)
  if (!target) target = selectComponents.find(item => item.__config__.tagIcon === tagIcon)
  emit('tag-change', target)
}

const changeRenderKey = () => {
  if (needRerenderList.includes(props.activeData.__config__.tag)) {
...
const onSwitchValueInput = (val: any, name: any) => {
...
const onDefaultValueInput = (str: string) => {
  if (Array.isArray(props.activeData.__config__.defaultValue)) {
    // 数组
    props.activeData.__config__.defaultValue = str.split(',').map(val => (isNumberStr(val) ? +val : val)
    )
...
const openIconsDialog = (model: any) => {
  // 打开图标
  // iconsVisible = true
...
const setTimeValue = (val: any, type: any) => {
  const valueFormat = type === 'week' ? dateTimeFormat.date : val
  props.activeData.__config__.defaultValue = null
  props.activeData.__config__['value-format'] = valueFormat
  props.activeData.__config__.format = val
}
...
const dateOptions = computed(() => {
  if (
    ...
    && props.activeData.__config__.tag === 'el-date-picker'
  ) {
    if ((props.activeData as any)['start-placeholder'] === undefined) {
...
const setOptionValue = (item: any, val: any) => {
...
const rateTextChange = (val: any) => {
  if (val) props.activeData['show-score'] = false
}

const rateScoreChange = (val: any) => {
  if (val) props.activeData['show-text'] = false
}

const rangeChange = (val: any) => {
  props.activeData.__config__.defaultValue = val ? [props.activeData.min, props.activeData.max] : props.activeData.min
}

const colorFormatChange = (val: any) => {
  props.activeData.__config__.defaultValue = null
  props.activeData['show-alpha'] = val.indexOf('a') > -1
  props.activeData.__config__.renderKey = +new Date() // 更新renderKey,重新渲染该组件
}
      ...
      <el-scrollbar class="right-scrollbar">
        ...
        <el-form v-if="showField" v-show="currentTab === 'field' && showField" size="small" label-width="90px">
          ...
          <el-form-item v-if="activeData.format !== undefined" label="时间格式">
            <el-input :value="activeData.format" placeholder="请输入时间格式" @input="setTimeValue($event, null)" />
          </el-form-item>
